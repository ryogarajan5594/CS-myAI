import { AI_NAME, OWNER_NAME } from "@/configuration/identity";

export const CHAT_HEADER: string = `HiredDSA AI`;
export const MESSAGE_PLACEHOLDER: string = `[MESSAGE PLACEHOLDER]`;
export const FOOTER_MESSAGE: string = `Customized AI by Raj Yogarajan @UNC`;
export const CLEAR_BUTTON_TEXT: string = `New Prep`;
export const PAGE_TITLE: string = `HiredDSA`;
export const PAGE_DESCRIPTION: string = `Chat with ${HIREDDSA}, ${Raj-Yogarajan}'s AI assistant.`;

export const EMPTY_CITATION_MESSAGE: string = "Unspecified source";
